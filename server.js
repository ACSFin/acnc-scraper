...
app.post("/fetchAcncFinancials", async (req, res) => {
  ...
  try {
    // 1) Search by ABN
    await page.goto(`https://www.acnc.gov.au/charity/charities?search=${abn}`, { waitUntil: "domcontentloaded", timeout: 60000 });

    // 2) Click first result row that matches ABN column
    const spaced = spaceAbn(abn);
    const row = page.locator("table tbody tr").filter({ has: page.locator(`td:last-child:has-text("${spaced}"), td:last-child:has-text("${abn}")`) }).first();
    ...
    for (let i = 0; i < count; i++) {
      ...
      const type = /annual information statement/i.test(title) ? "AIS" : "Financial Report";
    ...
    if (latestFr) {
      ...
      if (r.ok) {
        ...
        frTextPreview = (parsed.text || "").slice(0, 800);
      }
    }
